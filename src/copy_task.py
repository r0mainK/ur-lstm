...
def main() -> None:
    conf = OmegaConf.load(Path(__file__).parent / "configs" / "copy_config.yaml")
    conf.model_type = ModelType(conf.model_type)
    writer = create_writer(Path(f"copy-task-{conf.time_lag}"), conf.model_type, conf.seed)
    device = torch.device("cuda") if torch.cuda.is_available() else "cpu"
    torch.random.manual_seed(conf.seed)
    model = CopyModel(conf.model_type, conf.hidden_size, conf.forget_bias)
    model.to(device)
    loss_fn = nn.CrossEntropyLoss()
    optimizer = AdamW(model.parameters(), lr=conf.learning_rate)
    torch.random.manual_seed(conf.seed)
    dataset = CopyDataset(conf.time_lag)
    ...
    for step, (x, target) in tqdm.tqdm(enumerate(generator), desc="step", total=conf.n_steps):
        ...
        target = target.t().reshape(-1).to(device)
        model.zero_grad()
        optimizer.zero_grad()
        output = model(x)
        output = output[-10:].reshape(-1, 10)
        loss = loss_fn(output, target)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.lstm.parameters(), 1)
        optimizer.step()
        writer.add_scalar("Cross Entropy", loss.item(), step)
